...
<div class="bbk" style="height:900px; width:70%; top:50px; left: 520px;" >
    ...
    <form action="../back/view.php" method = "post">
        <div style="font-size: 30px; font-family: 'Adobe 楷体 Std R'">学会号:
            <input type = "text" name = "assno">
        </div>
    ...
    $mfgh=new mysql();
    $result =  $mfgh->View($assno);
    if($result instanceof mysqli_result){
        echo "The following are the results", "<br/>";
        echo '<center><table class="imagetable">
            <tr>
                <th>学会号</th><th>学会名</th><th>学会人数</th>
            </tr>';
        while($row = $result->fetch_object()){
            echo '<tr>
                <td>', $row->assno, '</td><td>', $row->assname, '</td><td>', $row->stunum, '</td>
            </tr>';
        }
        echo '</center>';
    }
